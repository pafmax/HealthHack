...
#read it
ids <- read_csv("~/Downloads/Publication.csv")
...
write_csv(article, "~/Downloads/Article.csv")



# article_authors <- rbind.pages(authors)
# article_type <- rbind.pages(types)
# article_keywords <- rbind.pages(keys)
# 
# mydata <-content(GET(paste0("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pubmed&retmode=xml&rettype=abstract&id=", 22581272)))
# datas <- parseXMLAndAdd(mydata)
# ## End(Not run)
# 
# json_data <- cbind(id[i, ], title, abstract, date_created)
# 
# data <- xmlToDataFrame(mydata)
# 
# xml <-  xmlTreeParse(mydata)
# xml <- xmlParse(mydata)
# 
# 
# mydata <- read_xml(mydata)
# data <- as.list.data.frame(xmlfile$xml_document)
# class(xml)
# 
# test <- (xpathApply(xml,"//*/PubmedArticle",xmlValue))
# test2 <- as.data.frame(t(xmlSApply(xml["//*/Abstract"],xmlValue)),
#               stringsAsFactors=FALSE)
# 
# xmlTreeParse(substr(xml,4,nchar(xml)))
# 
# title <- xpathSApply(xml, "//PubmedArticle", xmlValue)
# as_data_frame(mydata)
# xmlParse(mydata)
# xml_contents(xml)
# data2 <- xmlToList(mydata)
# 
# toJSON(xmlParse(mydata), pretty=TRUE)
# xml_children(mydata)
# 
# as.data.frame(mydata[1])
# xml_contents(mydata$doc)
# xmlName(mydata) #give name of node, PubmedArticleSet
# xmlSize(mydata) #how many children in node, 19
# xmlName(mydata[[1]]) #name of root's children
# data <- lapply(xmlToList("/Volumes/320GB/HealthHack/sample_submission_set.xml"), data.frame)
